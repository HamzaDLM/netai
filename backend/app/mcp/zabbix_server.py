...
class ZabbixSettings:
    """Standalone Zabbix connection settings loaded from environment variables."""
    ...
    @classmethod
    def from_environment(cls) -> "ZabbixSettings":
        return cls(
            enabled=os.getenv("ZABBIX_ENABLED", "true").lower()
            in {"1", "true", "yes", "on"},
            api_url=os.getenv("ZABBIX_API_URL", "").strip(),
            api_token=os.getenv("ZABBIX_API_TOKEN", "").strip(),
            timeout_seconds=float(os.getenv("ZABBIX_TIMEOUT_SECONDS", "12")),
        )


zabbix_settings = ZabbixSettings.from_environment()
mcp = FastMCP(
    name="NetAI Zabbix",
    instructions=(
        "Read-only Zabbix monitoring tools for host discovery, active problems, "
        "metrics, events, inventory, and evidence-based diagnosis."
    ),
)
...
class ZabbixGateway:
    ...
    def __init__(self) -> None:
        if not zabbix_settings.enabled:
            raise ZabbixToolError("zabbix_disabled")
        if ZabbixAPI is None:
            raise ZabbixToolError("pyzabbix_not_installed")
        if not zabbix_settings.api_url:
            raise ZabbixToolError("missing_zabbix_api_url")
        if not zabbix_settings.api_token:
            raise ZabbixToolError("missing_zabbix_api_token")

        self.api = ZabbixAPI(zabbix_settings.api_url)
        self.api.timeout = zabbix_settings.timeout_seconds
        self._login_with_token(zabbix_settings.api_token)
...
@mcp.tool(name="zabbix_get_hosts")
def get_hosts(
...
@mcp.tool(name="zabbix_get_host_details")
...
@mcp.tool(name="zabbix_get_host_interfaces")
...
@mcp.tool(name="zabbix_get_host_groups")
...
@mcp.tool(name="zabbix_get_hosts_in_group")
...
@mcp.tool(name="zabbix_get_problems")
def get_problems(
...
@mcp.tool(name="zabbix_get_recent_problems")
...
@mcp.tool(name="zabbix_get_host_problems")
...
@mcp.tool(name="zabbix_get_trigger_problems")
...
@mcp.tool(name="zabbix_get_triggers")
def get_triggers(
    hostname_or_ip: Annotated[str, "Target device hostname or IP"],
    min_severity: Annotated[
        str | None,
...
@mcp.tool(name="zabbix_get_trigger_details")
...
@mcp.tool(name="zabbix_get_latest_metrics_data")
...
@mcp.tool(name="zabbix_get_metrics_history")
...
@mcp.tool(name="zabbix_get_host_metrics_summary")
...
@mcp.tool(name="zabbix_get_events")
def get_events(
    hostname_or_ip: Annotated[
        str | None,
...
@mcp.tool(name="zabbix_get_audit_log")
def get_audit_log(
    hours: Annotated[int | float | None, "Lookback window in hours"] = DEFAULT_HOURS,
...
@mcp.tool(name="zabbix_get_host_templates")
...
@mcp.tool(name="zabbix_get_maintenance")
def get_maintenance(
    hostname_or_ip: Annotated[
        str | None,
...
@mcp.tool(name="zabbix_get_proxies")
def get_proxies(
    limit: Annotated[int, "Maximum rows to return"] = DEFAULT_LIMIT,
) -> dict[str, Any]:
...
@mcp.tool(name="zabbix_get_zabbix_server_status")
...
@mcp.tool(name="zabbix_diagnose_host")
def diagnose_host(
    hostname_or_ip: Annotated[str, "Target device hostname or IP"],
    hours: Annotated[int | float | None, "Lookback window in hours"] = DEFAULT_HOURS,
) -> dict[str, Any]:
...
@mcp.tool(name="zabbix_get_dashboard_snapshot")
...
def main() -> None:
    mcp.run(
        ...
        port=int(os.getenv("MCP_PORT", "8030")),
    )
