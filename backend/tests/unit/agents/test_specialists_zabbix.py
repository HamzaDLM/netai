...
def test_zabbix_specialist_toolset_is_populated_with_new_api_surface() -> None:
    ...
    assert "zabbix.get_hosts" in tool_names
    assert "zabbix.get_host_details" in tool_names
    assert "zabbix.diagnose_host" in tool_names
    assert "zabbix.get_zabbix_server_status" in tool_names

    # legacy names removed from exposed surface
    assert "zabbix.list_hosts" not in tool_names
    assert "zabbix.get_host_status" not in tool_names
    assert "zabbix.get_problem_summary" not in tool_names


def test_tools_prompt_zabbix_section_uses_new_names_only() -> None:
    assert "zabbix.get_hosts(" in TOOLS_PROMPT
    assert "zabbix.get_host_details(" in TOOLS_PROMPT
    assert "zabbix.diagnose_host(" in TOOLS_PROMPT

    assert "zabbix.list_hosts(" not in TOOLS_PROMPT
    assert "zabbix.get_host_status(" not in TOOLS_PROMPT
    assert "zabbix.get_problem_summary(" not in TOOLS_PROMPT
